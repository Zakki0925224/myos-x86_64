...
def task_build_cozette():
    ...
    if not os.path.exists(f"./{THIRD_PARTY_DIR}/{COZETTE_FILE}"):
        ...
        run_cmd("pipenv install --python 3 && pipenv run python3 ./build.py fonts", dir=d)
        run_cmd(f"bdf2psf --fb ./build/cozette.bdf /usr/share/bdf2psf/standard.equivalents /usr/share/bdf2psf/fontsets/Uni2.512 512 ../{COZETTE_FILE}", dir=d)
...
def task_build_qemu():
    ...
    if not os.path.exists(f"{d}/build/{QEMU_ARCH}"):
        run_cmd(f"{GIT_CHECKOUT_TO_LATEST_TAG}", dir=d)
        run_cmd(f"mkdir -p build && cd build && ../configure --target-list={QEMU_TARGET_ARCH} --enable-trace-backends=log --enable-sdl --extra-cflags=\"-DDEBUG_XHCI\" && make -j$(nproc)", dir=d)
...
def task_build_bootloader():
    ...
    run_cmd(f"cp ./target/x86_64-unknown-uefi/debug/bootloader.efi ../{OUTPUT_DIR}/{BOOTLOADER_FILE}", d)
...
def task_makeimg():
    ...
    run_cmd(f"sudo cp ./{OUTPUT_DIR}/{BOOTLOADER_FILE} {MNT_DIR_PATH}/EFI/BOOT/BOOTX64.EFI")
...
def task_dump():
    ...
    run_cmd(f"objdump -d ./{OUTPUT_DIR}/{BOOTLOADER_FILE} > ./{DUMP_DIR}/dump_bootloader.txt")
